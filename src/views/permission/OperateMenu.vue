<template>
    <section>
        ...
        <!--列表-->
        <el-table :data="operateMenus" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
            <el-table-column type="selection" width="55">
            </el-table-column>
            <el-table-column type="index" width="auto">
            </el-table-column>
            <el-table-column prop="name" label="菜单名" width="auto" sortable>
            </el-table-column>
            <el-table-column prop="url" label="访问标识" width="auto" sortable>
            </el-table-column>
            <el-table-column prop="icon" label="菜单图标" width="auto" sortable>
            </el-table-column>
            <el-table-column prop="parentId" label="父级菜单" width="auto" sortable>
            </el-table-column>
        ...
        <el-dialog title="编辑" v-bind:visible.sync="saveFormVisible" :close-on-click-modal="false">
            <el-form :model="saveForm" label-width="80px" :rules="saveFormRules" ref="editForm">
                ...
                <el-form-item label="访问标识" prop="url">
                    <el-input v-model="saveForm.url" auto-complete="on"></el-input>
                </el-form-item>
                <el-form-item label="菜单图标" prop="icon">
                    <el-input v-model="saveForm.icon" auto-complete="on"></el-input>
                </el-form-item>
                <el-form-item label="父级菜单" prop="parentId">
                    <el-input v-model="saveForm.parentId" auto-complete="on"></el-input>
                </el-form-item>
            </el-form>
...
export default {
    ...
    methods: {
        ...
        handleCurrentChange(val) {
            ...
            this.getAllOperateMenus()
        },
        //获取角色列表
        getAllOperateMenus() {
            this.listLoading = true
            this.$http.post("/OperateMenu/pageList",this.queryObj)
                .then(result => {
                    result = result.data
                    console.log("result", result)
                    this.operateMenus = result.resultObj.rows
                ...
                if (valid) {
                    this.$confirm('确认提交吗？', '提示', {}).then(() => {
                        ...
                        if (this.saveForm.id == ''|| this.saveForm.id == null){
                            this.$http.post('/OperateMenu/addOperateMenu',this.saveForm)
                                .then(result => {
                                    result = result.data
                                    this.$message({
                                        type:"success",
                            ...
                            this.$http.post('/OperateMenu/modifyOperateMenu',this.saveForm)
                                .then(result => {
                                    result = result.data
                                    this.$message({
                                        type:"success",
        ...
        handleDel: function (index, row) {
            this.$confirm('确认删除该显示菜单吗?', '提示', {
                type: 'warning'
            }).then(() => {
                this.listLoading = true;
                this.$http.delete("/OperateMenu/deleteById/"+row.id)
                    .then(result => {
                        this.$message({
                            type:"success",
        ...
        batchRemove: function () {
            ...
            this.$confirm('确认删除选中记录吗？', '提示', {
                ...
                // let idsObj = {ids:ids}
                this.$http.patch("/OperateMenu/patchDelete",ids)
                    .then(result =>{
                        result = result.data
                        this.$message({
                            type:"success",
